...
@app.on_message(command(["تلغراف", "تلغراف ميديا", "ميديا"]) )
...
@app.on_message(command(["معلوماته", "هوو"]) & filters.group ) 
async def hshs(client: Client, message: Message):      
    ...
    async for photo in client.iter_profile_photos(message.reply_to_message.from_user.id, limit=1):
                    await message.reply_photo(photo.file_id,       caption=f"""**[𝚂𝙾𝚄𝚁𝚂 𝙰𝙵𝚁𝙾𝚃𝙾𝙾 ‌♬⁩˼⁩](https://t.me/T_Y_E_X)\n\n🐉 ¦ ɴᴀᴍᴇ : {name}\n🤡 ¦ ᴜѕᴇ : {username}\n🔥 ¦ ɪᴅ : `{user_id}`\n🔅 ¦ ɪᴅ ᴄʜᴀᴛ : `{chat_idd}`\n💭 ¦ ᴄʜᴀᴛ : {chat_name}\n🐊 ¦ ɢʀᴏᴜᴘ : {chat_username} \n**""", 
        reply_markup=InlineKeyboardMarkup(
            [
                [
...
@app.on_message(command(["الجروب", "جروب","انا فين"]) & filters.group )
